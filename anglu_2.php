        ...
        <body>
            <table class="center"><tr><td>
				...
				<?php
						...
						$record=mysqli_query($dbc,"SELECT * from zodynas_anglu WHERE tematika='transportas'");
						...
						for ($i = 0; $i<count($zodziai)/2;$i++){
							...
							if($suvesti[$i]!= $laikinas[$i]['vertimas']){
								...
								$_SESSION['visia']=count($zodziai)/2;
						...
						if(empty($laikinas2) && empty($laikinas3) && $err==count($zodziai)/2) header("Location: anglu_teisingi.php");
						else if (!empty($laikinas2) && !empty($laikinas3) && $err==count($zodziai)/2) header("Location: anglu_antras.php");
