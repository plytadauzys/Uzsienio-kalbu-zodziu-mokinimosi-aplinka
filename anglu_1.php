        ...
        <body>
            <table class="center"><tr><td>
				...
				<?php
						...
						$query="DELETE FROM anglu_laikinas";
						mysqli_query($dbc, $query);
						shuffle($zodziai);
						$idetilaikinus=array();
						...
						for ($i = 0; $i<round(count($zodziai)/2);$i++){
							...
							if($result->num_rows == 0){
								$sql="INSERT INTO atsakyti (user, zodis, vertimas, tematika, atsakyta, atsakytateisingai) Values ('$session->username','$suvzod','$suvvert','literatura',0,0)";
								mysqli_query($dbc,$sql);
							}
							...
							if($suvesti[$i] != $laikinas[$i]['vertimas']){
								...
								$_SESSION['visia']=round(count($zodziai)/2);
								echo "yra neteisingu";
								...
								mysqli_query($dbc,$sql); echo "yra teisingi";
							}
						  }
						}
						/*echo "<pre>";
						...
						//echo $err;
						if(empty($laikinas2) && empty($laikinas3) && $err==round(count($zodziai)/2)) header("Location: anglu_teisingi.php");
						else if (!empty($laikinas2) && !empty($laikinas3) && $err==round(count($zodziai)/2)) header("Location: anglu_antras.php");
